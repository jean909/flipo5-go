...
export async function fetchBlobForJobRef(ref: string): Promise<Blob> {
  ...
  if (ref.startsWith('https://') || ref.startsWith('http://')) {
    return downloadMediaUrl(ref);
  }
  const res = await fetch(`${API_URL}/api/media?key=${encodeURIComponent(ref)}`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!res.ok) throw new Error('Download failed');
  return res.blob();
...
export async function createJobShare(jobId: string, expiresInDays?: number): Promise<CreateJobShareResult> {
  const token = await getToken();
  if (!token) throw new Error('Not logged in');
  const res = await fetch(`${API_URL}/api/shares`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ job_id: jobId, expires_in_days: expiresInDays ?? 7 }),
  });
  if (!res.ok) {
    const err = await res.text().catch(() => '');
    throw new Error(err || 'Share failed');
  }
  return res.json() as Promise<CreateJobShareResult>;
}

export async function revokeJobShare(shareId: string): Promise<void> {
  const token = await getToken();
  if (!token) throw new Error('Not logged in');
  const res = await fetch(`${API_URL}/api/shares/${encodeURIComponent(shareId)}`, {
    method: 'DELETE',
    headers: { Authorization: `Bearer ${token}` },
  });
...
export async function fetchPublicShare(shareToken: string): Promise<PublicSharePayload> {
  const res = await fetch(`${API_URL}/api/public/share/${encodeURIComponent(shareToken)}`);
  if (!res.ok) throw new Error('not found');
  return res.json() as Promise<PublicSharePayload>;
}

export function getPublicShareMediaUrl(shareToken: string, key: string): string {
  return `${API_URL}/api/public/share/${encodeURIComponent(shareToken)}/media?key=${encodeURIComponent(key)}`;
}

export function getPublicShareProxyUrl(shareToken: string, mediaUrl: string): string {
  return `${API_URL}/api/public/share/${encodeURIComponent(shareToken)}/proxy?url=${encodeURIComponent(mediaUrl)}`;
}

/** Read-only share page: stream media without login (signed token). */
export function getShareDisplayUrl(ref: string, shareToken: string): string {
  if (!ref) return '';
  if (ref.startsWith('data:')) return ref;
  if (ref.startsWith('http://') || ref.startsWith('https://')) {
    return getPublicShareProxyUrl(shareToken, ref);
  }
  return getPublicShareMediaUrl(shareToken, ref);
}
