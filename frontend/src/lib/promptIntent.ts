/**
 * Heuristic intent from free-text prompts: image vs video vs chat, regenerate phrases, and image URL extraction from job input.
 * Used by the dashboard to suggest mode and handle attachments; tuned for EN/DE phrasing.
 */
...
/** Returns dominant media intent or null when ambiguous / chat-like. */
...
/** Collects reference image URLs from persisted job input shapes (`imageInput`, snake_case, single `image`, `source_url`). */
