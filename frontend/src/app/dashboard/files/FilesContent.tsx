...
/** Stale-while-revalidate TTL for My Files lists (45s). */
const FILES_CACHE_MS = 45_000;
...
export default function FilesContent() {
  ...
  const isCacheFresh = useCallback(() => Date.now() - listCacheRef.current.at < FILES_CACHE_MS, []);
  const invalidateListCache = useCallback(() => {
    listCacheRef.current.at = 0;
  }, []);

  const fetchFiles = useCallback(() => {
    const cache = listCacheRef.current;
    if (cache.files != null && isCacheFresh()) {
      setFiles(cache.files);
      listFiles()
        .then((r) => {
          const next = r.files ?? [];
          listCacheRef.current = { ...listCacheRef.current, files: next, at: Date.now() };
          setFiles(next);
        })
        .catch(() => {});
      return;
  ...
  const fetchProjects = useCallback(() => {
    const cache = listCacheRef.current;
    if (cache.projects != null && isCacheFresh()) {
      setProjects(cache.projects);
      listTranslationProjects()
        .then((r) => {
          const next = r.projects ?? [];
          listCacheRef.current = { ...listCacheRef.current, projects: next, at: Date.now() };
          setProjects(next);
        })
        .catch(() => {});
      return;
  ...
  const fetchLogos = useCallback(() => {
    const cache = listCacheRef.current;
    if (cache.logoJobs != null && isCacheFresh()) {
      setLogoJobs(cache.logoJobs);
      listContent({ type: 'logo', limit: 50 })
        .then((r) => {
          const next = r.jobs ?? [];
          listCacheRef.current = { ...listCacheRef.current, logoJobs: next, at: Date.now() };
          setLogoJobs(next);
        })
        .catch(() => {});
      return;
  ...
  const fetchProducts = useCallback(() => {
    const cache = listCacheRef.current;
    if (cache.products != null && isCacheFresh()) {
      setProducts(cache.products);
      listProducts()
        .then((r) => {
          const next = r.products ?? [];
          listCacheRef.current = { ...listCacheRef.current, products: next, at: Date.now() };
          setProducts(next);
        })
        .catch(() => {});
      return;
              ...
              return (
                <div key={project.id} className="rounded-xl border border-theme-border overflow-hidden mb-1.5 bg-theme-bg">
                  <button type="button" onClick={() => toggleProject(project.id)}
                    className="w-full text-left flex items-center gap-2 px-3 py-2.5 rounded-t-xl hover:bg-theme-bg-hover transition-colors">
